...
class IsaacsimHandler(BaseSimHandler):
    ...
    def _init_scene(self, simulation_app=None, args=None) -> None:
        ...
        if simulation_app is None:
            ...
            # For joint_pos-only runs, avoid loading rendering-heavy camera stack.
            args.enable_cameras = len(self.cameras) > 0
        ...
        sim_device = os.getenv("METASIM_ISAACSIM_DEVICE")
        if not sim_device:
            sim_device = self._auto_select_cuda_device()
    ...
    @staticmethod
    def _auto_select_cuda_device() -> str:
        """Pick the CUDA device with the most free memory, fallback to cuda:0."""
        if not torch.cuda.is_available():
            return "cpu"

        # Prefer torch runtime query (respects CUDA_VISIBLE_DEVICES remapping).
        try:
            ...
            for i in range(n):
                ...
                if int(free_bytes) > best_free:
                    ...
                    best_idx = i
            if best_free > 0:
                log.info(f"[IsaacSim] Auto-selected device cuda:{best_idx}, free={best_free / (1024**3):.2f} GB")
                return f"cuda:{best_idx}"
        except Exception as err:
            log.debug(f"[IsaacSim] torch-based device probe failed: {err}")
        ...
        try:
            ...
            if best_free_mb > 0:
                log.info(f"[IsaacSim] Auto-selected device cuda:{best_idx} via nvidia-smi, free={best_free_mb} MB")
                return f"cuda:{best_idx}"
        except Exception as err:
            log.debug(f"[IsaacSim] nvidia-smi probe failed: {err}")
    ...
    def launch(self, simulation_app=None, simulation_args=None) -> None:
        def _is_dof_backend_error(err: Exception) -> bool:
            msg = str(err)
            return ("Failed to get DOF velocities from backend" in msg) or ("get_dof_velocities" in msg)

        def _hard_recover_assets() -> None:
            log.warning("[IsaacLaunch] trying hard recovery for PhysX tensor backend ...")
            try:
                self.sim.reset(soft=False)
            except Exception as recover_err:
                log.warning(f"[IsaacLaunch] hard reset during recovery failed: {recover_err}")
            self._wait_for_scene_assets_ready(max_steps=180)

        log.info("[IsaacLaunch] _init_scene")
        self._init_scene(simulation_app, simulation_args)
        log.info("[IsaacLaunch] _load_robots")
        self._load_robots()
        log.info("[IsaacLaunch] _load_sensors")
        self._load_sensors()
        log.info("[IsaacLaunch] _load_cameras")
        self._load_cameras()
        if self.scenario.scene is None:
            log.info("[IsaacLaunch] _load_terrain")
            self._load_terrain()
        log.info("[IsaacLaunch] _load_scene")
        self._load_scene()
        log.info("[IsaacLaunch] _load_objects")
        self._load_objects()
        log.info("[IsaacLaunch] _load_lights")
        self._load_lights()
        if len(self.cameras) > 0:
            log.info("[IsaacLaunch] _load_render_settings")
            self._load_render_settings()
        else:
            # joint_pos-only eval path does not need RTX render settings
            log.info("[IsaacLaunch] skip _load_render_settings (no cameras)")
        log.info("[IsaacLaunch] clone_environments")
        self.scene.clone_environments(copy_from_source=False)
        log.info("[IsaacLaunch] filter_collisions")
        self.scene.filter_collisions(global_prim_paths=["/World/ground"])
        self._ensure_active_physics_scene()
        skip_first_reset = os.getenv("METASIM_ISAACSIM_SKIP_FIRST_RESET", "0") == "1"
        soft_first_reset = os.getenv("METASIM_ISAACSIM_SOFT_RESET", "0") == "1"
        if skip_first_reset:
            log.warning("[IsaacLaunch] skip sim.reset (1) due to METASIM_ISAACSIM_SKIP_FIRST_RESET=1")
            log.info("[IsaacLaunch] play/warmup and wait for scene assets (skip-first-reset path)")
            log.info("[IsaacLaunch] warmup begin")
            self._kickoff_playback_warmup()
            log.info("[IsaacLaunch] warmup end; begin wait loop")
            if not self._wait_for_scene_assets_ready(max_steps=240):
                log.warning("[IsaacLaunch] asset wait timed out on skip-first-reset path")
        else:
            log.info(f"[IsaacLaunch] sim.reset (1), soft={soft_first_reset}")
            self.sim.reset(soft=soft_first_reset)
            # Wait until runtime handles are actually present before scene.reset/update.
            if not self._wait_for_scene_assets_ready(max_steps=120):
                log.warning("[IsaacLaunch] assets not ready after first reset; try hard reset fallback")
                self.sim.reset(soft=False)
                self._wait_for_scene_assets_ready(max_steps=120)
        indices = torch.arange(self.num_envs, dtype=torch.int64, device=self.device)
        if skip_first_reset:
            log.info("[IsaacLaunch] skip early scene.reset (will do late reset after first sim.step)")
        else:
            log.info("[IsaacLaunch] scene.reset")
            try:
                self.scene.reset(indices)
            except AttributeError as err:
                if "actuators" in str(err):
                    log.warning("[IsaacLaunch] scene.reset hit missing `actuators`; waiting and retrying once")
                    self._wait_for_scene_assets_ready(max_steps=120)
                    self.scene.reset(indices)
                else:
                    raise
            except Exception as err:
                if _is_dof_backend_error(err):
                    log.warning(f"[IsaacLaunch] scene.reset hit DOF backend error: {err}")
                    _hard_recover_assets()
                    self.scene.reset(indices)
                else:
                    raise

        # Update camera pose after scene reset to avoid being overridden
        log.info("[IsaacLaunch] _update_camera_pose")
        self._update_camera_pose()

        # Force another simulation step and camera update to ensure proper initialization
        log.info("[IsaacLaunch] sim.step(render=False)")
        self.sim.step(render=False)
        log.info("[IsaacLaunch] scene.update")
        try:
            self.scene.update(dt=self.physics_dt)
        except AttributeError as err:
            if "_data" in str(err):
                log.warning("[IsaacLaunch] scene.update hit missing `_data`; waiting and retrying once")
                self._wait_for_scene_assets_ready(max_steps=120)
                self.scene.update(dt=self.physics_dt)
            else:
                raise
            ...
            if _is_dof_backend_error(err):
                log.warning(f"[IsaacLaunch] scene.update hit DOF backend error: {err}")
                _hard_recover_assets()
                self.scene.update(dt=self.physics_dt)
            else:
                raise
        if skip_first_reset:
            log.info("[IsaacLaunch] late scene.reset")
            try:
                self.scene.reset(indices)
            except AttributeError as err:
                # Compatibility fallback for IsaacLab versions where some articulations
                # (e.g., passive objects) may not expose `actuators` at reset time.
                if "actuators" in str(err):
                    log.warning(
                        "[IsaacLaunch] late scene.reset hit missing `actuators`; "
                        "injecting empty actuator dict for passive articulations and retrying."
                    )
                    for articulation in self.scene.articulations.values():
                        if not hasattr(articulation, "actuators"):
                ...
                if _is_dof_backend_error(err):
                    log.warning(f"[IsaacLaunch] late scene.reset hit DOF backend error: {err}")
                    _hard_recover_assets()
                    self.scene.reset(indices)
                else:
                    raise
        ...
        if self.sim.has_gui() or self.sim.has_rtx_sensors():
            log.info("[IsaacLaunch] sim.render")
            self.sim.render()
        log.info("[IsaacLaunch] sensor.update")
        for sensor in self.scene.sensors.values():
            sensor.update(dt=0)

        # Initialize GS background if enabled
        log.info("[IsaacLaunch] _build_gs_background")
        self._build_gs_background()
        log.info("[IsaacLaunch] super().launch")
        super().launch()
        log.info("[IsaacLaunch] sim.reset (2), soft=True")
        self.sim.reset(soft=True)  # keep lightweight to avoid reset hang on some setups
        log.info("[IsaacLaunch] done")

    def _force_initialize_scene_assets(self) -> None:
        """Best-effort fallback for setups where first sim.reset hangs.

        IsaacLab assets typically initialize on timeline PLAY/reset callbacks.
        If reset is skipped, some assets may miss fields like `actuators` / `_data`.
        """
        def _is_ready(asset) -> bool:
            # For articulations we require runtime fields to exist.
            if asset.__class__.__name__ == "Articulation":
                return hasattr(asset, "actuators") and hasattr(asset, "_data")
            return getattr(asset, "is_initialized", False)

        def _try_init(asset, name: str) -> None:
            if getattr(asset, "is_initialized", False):
                if _is_ready(asset):
                    return
            cb = getattr(asset, "_initialize_callback", None)
            impl = getattr(asset, "_initialize_impl", None)
            try:
                if cb is not None:
                    cb(None)
                # Some versions mark initialized late or miss runtime fields.
                if not _is_ready(asset) and impl is not None:
                    impl()
                    if hasattr(asset, "_is_initialized"):
                        asset._is_initialized = True
                if _is_ready(asset):
                    log.debug(f"[IsaacLaunch] initialized asset: {name}")
                else:
                    log.warning(f"[IsaacLaunch] asset not fully ready after init: {name}")
            except Exception as err:
                log.warning(f"[IsaacLaunch] failed to initialize asset {name}: {err}")

        for name, articulation in getattr(self.scene, "articulations", {}).items():
            _try_init(articulation, f"articulation:{name}")
        for name, deformable in getattr(self.scene, "deformable_objects", {}).items():
            _try_init(deformable, f"deformable:{name}")
        for name, rigid in getattr(self.scene, "rigid_objects", {}).items():
            _try_init(rigid, f"rigid:{name}")
        for name, coll in getattr(self.scene, "rigid_object_collections", {}).items():
            _try_init(coll, f"rigid_collection:{name}")
        for name, sensor in getattr(self.scene, "sensors", {}).items():
            _try_init(sensor, f"sensor:{name}")

    def _ensure_active_physics_scene(self) -> None:
        """Make sure PhysX scene is active before creating tensor views."""
        try:
            import omni.physx

            physx_iface = omni.physx.acquire_physx_interface()
            if physx_iface is not None:
                log.info("[IsaacLaunch] force_load_physics_from_usd")
                physx_iface.force_load_physics_from_usd()
        except Exception as err:
            log.warning(f"[IsaacLaunch] force_load_physics_from_usd failed: {err}")

        # Warm one light reset/step cycle to let physics scene become active.
        try:
            log.info("[IsaacLaunch] priming sim with soft reset")
            self.sim.reset(soft=True)
        except Exception as err:
            log.warning(f"[IsaacLaunch] soft reset prime failed: {err}")
        try:
            self.sim.step(render=False)
        except Exception:
            pass

    def _wait_for_scene_assets_ready(self, max_steps: int = 120) -> bool:
        """Wait for IsaacLab assets to finish runtime initialization."""

        def _asset_ready() -> bool:
            for articulation in getattr(self.scene, "articulations", {}).values():
                if not getattr(articulation, "is_initialized", False):
                    return False
                if not hasattr(articulation, "actuators") or not hasattr(articulation, "_data"):
                    return False
            for rigid in getattr(self.scene, "rigid_objects", {}).values():
                if not getattr(rigid, "is_initialized", False):
                    return False
            for sensor in getattr(self.scene, "sensors", {}).values():
                if hasattr(sensor, "is_initialized") and not getattr(sensor, "is_initialized", False):
                    return False
            return True
        ...
        use_step_in_wait = os.getenv("METASIM_ISAACSIM_WAIT_USE_STEP", "1") == "1"
        for step_idx in range(max_steps):
            # Keep timeline alive and try to enter playing state.
            if step_idx % 10 == 0:
                self._kickoff_playback_warmup()

            # Avoid self.sim.step() by default here because certain IsaacSim/driver
            # combos can block indefinitely before backend init completes.
            if use_step_in_wait:
                try:
                    self.sim.step(render=False)
                except Exception:
            ...
            if step_idx % 20 == 0:
                self._force_initialize_scene_assets()
                log.info(
                    f"[IsaacLaunch] waiting scene assets... step={step_idx + 1}/{max_steps}, "
                    f"use_step={use_step_in_wait}"
                )

            if _asset_ready():
                log.info(f"[IsaacLaunch] scene assets ready after wait steps={step_idx + 1}")
                return True

        log.warning("[IsaacLaunch] scene assets still not ready after wait")
        return False

    def _kickoff_playback_warmup(self) -> None:
        """Best-effort helper to move sim/timeline into a state where asset backends can initialize."""
        call_play = os.getenv("METASIM_ISAACSIM_WAIT_CALL_PLAY", "0") == "1"
        if call_play:
            try:
                play_fn = getattr(self.sim, "play", None)
                if callable(play_fn):
                    play_fn()
            except Exception:
                pass

        # A few app ticks help timeline callbacks and fabric state propagate.
        for _ in range(3):
            if not self._tick_app_once():
                break

    def _tick_app_once(self) -> bool:
        """Single non-blocking app tick with minimal rendering assumptions."""
        # Prefer raw app.update to avoid render-path dependencies (GLFW/GLInterop)
        # in headless no-camera evaluation.
        app = getattr(self.sim, "app", None)
        if app is not None:
            try:
                app.update()
                return True
    ...
    def _get_foreground_mask(
        ...
        if instance_id_seg_data is not None and instance_id_seg_id2label is not None:
            ...
            terrain_ids = {
                ...
                if any(kw in _label_text(label) for kw in ["ground", "terrain", "floor", "world/ground"])
            }
    ...
    @staticmethod
    def _labels_are_uninformative(id2label: dict | None) -> bool:
        """Return True when labels look like all 'unlabelled' / non-semantic."""
        ...
        for _, label in id2label.items():
            ...
            if any(tok in text for tok in ["unlabelled", "unknown", "background", "none", "other"]):
                generic += 1
                continue
            # Heuristic: object-like naming from prim path or explicit names.
            if any(tok in text for tok in ["/world/", "env_", "box", "lid", "drawer", "handle", "cube", "obj"]):
                informative += 1
                continue
            # If still non-empty and not generic, count as informative.
            if text.strip():
                informative += 1
        if informative == 0:
            return True
        # Mostly generic labels should still be considered uninformative.
        return informative <= max(1, generic // 4)
    ...
    def _get_states(self, env_ids: list[int] | None = None) -> TensorState:
        ...
        for camera in self.cameras:
            ...
            # Try non-fast annotators as fallback: in some IsaacSim scenes, "fast"
            # streams return only generic 'unlabelled' entries while non-fast carries
            # richer object labels usable for keyword matching.
            instance_seg_data_nf = deep_get(camera_inst.data.output, "instance_segmentation")
            instance_seg_id2label_nf = deep_get(camera_inst.data.info, "instance_segmentation", "idToLabels")
            instance_id_seg_data_nf = deep_get(camera_inst.data.output, "instance_id_segmentation")
            instance_id_seg_id2label_nf = deep_get(camera_inst.data.info, "instance_id_segmentation", "idToLabels")

            if (
                (instance_seg_data is None or self._labels_are_uninformative(instance_seg_id2label))
                and instance_seg_data_nf is not None
            ):
                instance_seg_data = instance_seg_data_nf
                instance_seg_id2label = instance_seg_id2label_nf
            if (
                (instance_id_seg_data is None or self._labels_are_uninformative(instance_id_seg_id2label))
                and instance_id_seg_data_nf is not None
            ):
                instance_id_seg_data = instance_id_seg_data_nf
                instance_id_seg_id2label = instance_id_seg_id2label_nf
    ...
    def _add_pinhole_camera(self, camera: PinholeCameraCfg) -> None:
        ...
        # NOTE:
        # Some IsaacSim/driver combinations may crash during renderer/plugin
        # startup when non-fast segmentation annotators are enabled.
        # Keep non-fast streams opt-in via env var for stability.
        enable_slow_seg = os.getenv("METASIM_ISAACSIM_ENABLE_SLOW_SEG", "0") in {"1", "true", "TRUE", "yes", "YES"}

        data_type_map = {
            "rgb": ["rgb"],
            "depth": ["depth"],
            "instance_seg": (
                ["instance_segmentation_fast", "instance_segmentation"]
                if enable_slow_seg
                else ["instance_segmentation_fast"]
            ),
            "instance_id_seg": (
                ["instance_id_segmentation_fast", "instance_id_segmentation"]
                if enable_slow_seg
                else ["instance_id_segmentation_fast"]
            ),
        }
        if camera.mount_to is None:
        ...
        camera_inst = TiledCamera(
            TiledCameraCfg(
                prim_path=prim_path,
                offset=offset,
                data_types=list(dict.fromkeys(sum([data_type_map[dt] for dt in camera.data_types], []))),
