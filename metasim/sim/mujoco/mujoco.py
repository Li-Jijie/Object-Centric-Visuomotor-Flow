...
class MujocoHandler(BaseSimHandler):
    ...
    def _build_geom_id2label(self, geom_ids: np.ndarray) -> dict[int, str]:
        id2label: dict[int, str] = {}
        if self._mj_model is None:
            return id2label
        for gid in np.unique(geom_ids):
            gid_i = int(gid)
            if gid_i < 0 or gid_i >= int(self._mj_model.ngeom):
                continue
            geom_name = mujoco.mj_id2name(self._mj_model, mujoco.mjtObj.mjOBJ_GEOM, gid_i) or f"geom_{gid_i}"
            body_id = int(self._mj_model.geom_bodyid[gid_i])
            body_name = ""
            if body_id >= 0:
                body_name = mujoco.mj_id2name(self._mj_model, mujoco.mjtObj.mjOBJ_BODY, body_id) or f"body_{body_id}"
            id2label[gid_i] = f"{body_name}/{geom_name}" if body_name else geom_name
        return id2label

    def _infer_geom_and_aux_seg_channels(self, sim_seg: np.ndarray) -> tuple[np.ndarray, np.ndarray | None]:
        """Infer which segmentation channel is geom id.

        dm_control/mujoco segmentation channel order can vary across builds.
        We pick the channel with higher ratio of ids within [0, ngeom).
        """
        if sim_seg.ndim != 3 or sim_seg.shape[-1] < 2:
            geom_ids = np.ascontiguousarray(sim_seg.astype(np.int32))
            return geom_ids, None

        ch0 = np.ascontiguousarray(sim_seg[..., 0].astype(np.int32))
        ch1 = np.ascontiguousarray(sim_seg[..., 1].astype(np.int32))
        ngeom = int(self._mj_model.ngeom) if self._mj_model is not None else 0
        if ngeom <= 0:
            return ch0, ch1

        valid0 = ((ch0 >= 0) & (ch0 < ngeom)).mean()
        valid1 = ((ch1 >= 0) & (ch1 < ngeom)).mean()
        if valid1 > valid0:
            return ch1, ch0
        return ch0, ch1
        ...
        for obj in self.objects:
            if isinstance(obj, (PrimitiveCubeCfg, PrimitiveCylinderCfg, PrimitiveSphereCfg)):
                ...
                if obj.mjcf_path is None:
                    raise RuntimeError(
                        f"Object '{obj.name}' has no mjcf_path configured. "
                        f"This task only supports simulators with USD assets (e.g. --sim=isaacsim). "
                        f"To use --sim=mujoco, an MJCF file must be provided for this object."
                    )
    ...
    def _get_states(self, env_ids: list[int] | None = None) -> list[dict]:
        ...
        for camera in self.cameras:
            ...
            depth = None
            instance_seg = None
            instance_id_seg = None
            instance_seg_id2label = None
            instance_id_seg_id2label = None
            cam_pos_t = None
            cam_intr_t = None
            ...
            if "instance_seg" in camera.data_types or "instance_id_seg" in camera.data_types:
                sim_seg = self.physics.render(
                    width=camera.width, height=camera.height, camera_id=camera_id, depth=False, segmentation=True
                )
                geom_ids, aux_ids = self._infer_geom_and_aux_seg_channels(sim_seg)

                if "instance_seg" in camera.data_types:
                    instance_seg = torch.from_numpy(geom_ids).unsqueeze(0)
                    instance_seg_id2label = self._build_geom_id2label(geom_ids)
                if "instance_id_seg" in camera.data_types:
                    if aux_ids is not None:
                        instance_id_seg = torch.from_numpy(aux_ids).unsqueeze(0)
                        instance_id_seg_id2label = {int(i): f"obj_{int(i)}" for i in np.unique(aux_ids) if int(i) >= 0}
                    else:
                        instance_id_seg = torch.from_numpy(geom_ids).unsqueeze(0)
                        instance_id_seg_id2label = self._build_geom_id2label(geom_ids)

            try:
                Ks, c2w = self._get_camera_params(camera_id, camera)
                cam_pos_t = torch.from_numpy(np.asarray(c2w[:3, 3], dtype=np.float32)).unsqueeze(0)
                cam_intr_t = torch.from_numpy(np.asarray(Ks, dtype=np.float32)).unsqueeze(0)
            except Exception:
                cam_pos_t = None
                cam_intr_t = None
