...
def state_tensor_to_nested(handler: BaseSimHandler, tensor_state: TensorState) -> list[DictEnvState]:
    ...
    for env_id in range(num_envs):
        ...
        for camera_name, camera_state in tensor_state.cameras.items():
            ...
            if camera_state.instance_seg is not None:
                cam_dict["instance_seg"] = camera_state.instance_seg[env_id].cpu()
            if camera_state.instance_id_seg is not None:
                cam_dict["instance_id_seg"] = camera_state.instance_id_seg[env_id].cpu()
            if camera_state.instance_seg_id2label is not None:
                cam_dict["instance_seg_id2label"] = camera_state.instance_seg_id2label
            if camera_state.instance_id_seg_id2label is not None:
                cam_dict["instance_id_seg_id2label"] = camera_state.instance_id_seg_id2label
            if camera_state.pos is not None:
                cam_dict["cam_pos"] = camera_state.pos[env_id].cpu()
            if camera_state.intrinsics is not None:
                cam_dict["cam_intr"] = camera_state.intrinsics[env_id].cpu()
