...
class RobotImageDataset(BaseImageDataset):
    def __init__(
        ...
        base_keys = ["head_camera", "state", "action"]
        optional_keys = ["task_mask", "dino_objfeat"]
        zarr_root = ReplayBuffer.create_from_path(zarr_path, mode="r")
        available_keys = set(zarr_root.data.keys())
        selected_keys = base_keys + [k for k in optional_keys if k in available_keys]
    ...
    def postprocess(self, samples, device):
        ...
        if "task_mask" in samples:
            # Binary task mask in [0, 1], shape: B, T, 1, H, W
            task_mask = samples["task_mask"].to(device, non_blocking=True).float()
            if task_mask.ndim == 4:
                task_mask = task_mask.unsqueeze(2)
            task_mask = (task_mask > 127.5).float()
            batch["task_mask"] = task_mask
        if "dino_objfeat" in samples:
            batch["dino_objfeat"] = samples["dino_objfeat"].to(device, non_blocking=True).float()
        return batch
