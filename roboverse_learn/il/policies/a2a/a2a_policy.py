...
class A2AImagePolicy(BaseImagePolicy):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if robot_dim is not None: action_dim = int(robot_dim)
        # get feature dim
        if robot_dim is not None:
            obs_encoder.shape_meta = shape_meta
        obs_feature_dim = obs_encoder.output_shape()[0]
        if robot_dim is not None:
            # Backward-compatible dimension alignment:
            # - old UR3 setup: shape_meta low-dim still 9, but robot_dim=7  -> subtract 2
            # - new UR3 setup: shape_meta low-dim already 7               -> subtract 0
            cfg_low_dim = int(shape_meta["obs"]["agent_pos"]["shape"][0])
            adjust_dim = max(0, cfg_low_dim - int(robot_dim))
            obs_feature_dim = obs_feature_dim - adjust_dim
    ...
    def compute_loss(self, batch):
        ...
        if nobs_features.shape[-1] != self.obs_feature_dim: nobs_features = torch.nn.functional.pad(nobs_features, (0, self.obs_feature_dim - nobs_features.shape[-1]))
        if nobs_features.shape[-1] != self.obs_feature_dim: nobs_features = torch.nn.functional.pad(nobs_features, (0, self.obs_feature_dim - nobs_features.shape[-1]))
        ...
        if history_states.shape[-1] < self.action_dim: history_states = torch.nn.functional.pad(history_states, (0, self.action_dim - history_states.shape[-1]))
        if history_states.shape[-1] < self.action_dim: history_states = torch.nn.functional.pad(history_states, (0, self.action_dim - history_states.shape[-1]))
        history_latents = self.history_action_encoder(history_states)
        start_latents = self._select_flow_start(history_latents)
        ...
        flow_loss, metrics = self.flow_matcher.compute_loss(
            ...
            start=start_latents,  # History states as flow source
