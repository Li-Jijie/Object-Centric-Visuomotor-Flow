...
class DefaultEvalRunner(BaseEvalRunner):
    ...
    def _init_policy(self, default_runner: DefaultRunner, **kwargs):
        ...
        # Always deserialize checkpoint storages on CPU first, then move policy to
        # the runtime device. This avoids CUDA ordinal mismatch when using
        # CUDA_VISIBLE_DEVICES (e.g., ckpt saved on cuda:4, current process sees only cuda:0).
        with open(kwargs["checkpoint_path"], "rb") as f:
            payload = torch.load(f, pickle_module=dill, map_location="cpu")
        cfg = payload["cfg"]
        # Eval does not need optimizer/scheduler states. Exclude them to avoid
        # param-group mismatch when model parameterization changed.
        state_keys = list(payload.get("state_dicts", {}).keys())
        exclude_keys = tuple(
            k
            for k in state_keys
            if ("optim" in k.lower()) or ("scheduler" in k.lower())
        )
        # Backward compatibility for checkpoint schema evolution:
        # allow missing newly-added optional parameters (e.g. obj_start_*).
        default_runner.load_payload(
            payload,
