...
class ReplayBuffer:
    ...
    def __init__(self, root: Union[zarr.Group, Dict[str, dict]]):
        ...
        for key, value in self._iter_items(root["data"]):
            assert value.shape[0] == root["meta"]["episode_ends"][-1]
        self.root = root

    @staticmethod
    def _iter_items(container):
        """Iterate key/value pairs across dict, zarr v2 Group and zarr v3 Group."""
        ...
        if hasattr(container, "items"):
            try:
                ...
                pass
        # zarr v3 Group may not expose items(); fall back to keys()+__getitem__
    ...
    def copy_from_store(
        ...
        **kwargs,
        ...
        Load to memory.
        """
        # Open source store in read-only mode for compatibility with zarr v3 read-only stores.
        src_root = zarr.open_group(store=src_store, mode="r")
        root = None
        if store is None:
            # numpy backend
            meta = dict()
            for key, value in cls._iter_items(src_root["meta"]):
    ...
    def save_to_store(
        ...
        **kwargs,
        ...
        if self.backend == "zarr":
            ...
            for key, value in self._iter_items(self.root["meta"]):
                _ = meta_group.array(name=key, data=value, shape=value.shape, chunks=value.shape)

        # save data, chunk
        data_group = root.create_group("data", overwrite=True)
        for key, value in self._iter_items(self.root["data"]):
    ...
    def values(self):
        return (v for _, v in self._iter_items(self.data))

    def items(self):
        return self._iter_items(self.data)
