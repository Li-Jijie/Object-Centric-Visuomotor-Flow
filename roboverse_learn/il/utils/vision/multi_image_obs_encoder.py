    ...
    def forward(self, obs_dict):
        ...
        if self.share_rgb_model:
            ...
            for key in self.rgb_keys:
                ...
                if img.shape[1:] != self.key_shape_map[key]:
                    img = torch.nn.functional.interpolate(img, size=self.key_shape_map[key][1:], mode="bilinear", align_corners=False)
        ...
        for key in self.low_dim_keys:
            ...
            if data.shape[1:] != self.key_shape_map[key]:
                ...
                lgr.warning(f'Shape mismatch for {key}: config expects {self.key_shape_map[key]} but data is {data.shape[1:]}. Auto-updating.')
                self.key_shape_map[key] = data.shape[1:]
