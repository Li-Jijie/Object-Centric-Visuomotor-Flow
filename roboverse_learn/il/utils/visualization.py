...
def plot_flow_trajectories(
    ...
    # Use PCA (linear, honest) instead of t-SNE (non-linear, can distort)
    ...
    # Styling (2x larger fonts, title unchanged)
    ax.set_xlabel('PCA Dimension 1', fontsize=56)
    ax.set_ylabel('PCA Dimension 2', fontsize=56)
...
def plot_all_latent_visualizations(
    ...
    # 2b. Baseline flow trajectories (gate=0, start=history)
    if trajectories_base is not None and trajectory_targets is not None:
        base_path = save_dir / f"epoch_{epoch:02d}_flow_trajectories_baseline.png"
        _, base_end_to_target = plot_flow_trajectories(
            trajectories_base, trajectory_targets, epoch, str(base_path),
            perplexity=perplexity, random_state=random_state
        )
        results['flow_trajectories_baseline'] = str(base_path)
        results['flow_end_to_target_dist_baseline'] = base_end_to_target
