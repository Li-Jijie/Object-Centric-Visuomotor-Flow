...
class Args:
    ...
    """Generate task-related mask during demo saving (optional)."""
    auto_enable_task_mask_for_isaac: bool = False
    """Auto-enable online task mask for IsaacSim/IsaacLab when not explicitly enabled."""
    task_mask_radius: int = 18
    """Pixel radius for projected-center fallback mask when segmentation is unavailable."""
    task_mask_allow_center_fallback: bool = False
    """Allow center-circle fallback when object mask extraction fails."""
    task_mask_debug: bool = True
    """Dump per-frame task mask debug info into metadata.json."""
    camera_width: int = 256
    """Camera image width for collection."""
    camera_height: int = 256
    """Camera image height for collection."""
    ...
    def __post_init__(self):
        ...
        # IsaacSim/IsaacLab collection usually needs task mask for A2A object-aware training.
        # Keep manual override available via --generate_task_mask.
        if self.sim in {"isaacsim", "isaaclab"} and self.auto_enable_task_mask_for_isaac and not self.generate_task_mask:
            self.generate_task_mask = True
            log.info("Auto-enabled task mask generation for IsaacSim/IsaacLab collection.")
        elif self.sim in {"isaacsim", "isaaclab"} and not self.generate_task_mask:
            log.info("Online task mask generation is disabled; you can run offline SAM3 postprocess later.")

        # IsaacSim level-1+ with raytracing can easily OOM during material randomization.
        # For data collection and mask generation, rasterization is sufficient and much cheaper.
        if self.sim in {"isaacsim", "isaaclab"} and self.level > 0 and self.render.mode == "raytracing":
            self.render.mode = "rasterization"
            log.warning("Auto-switch render.mode to rasterization for DR collection to avoid GPU OOM.")
...
def save_demo_mp(save_req_queue: mp.Queue, robot_cfg: RobotCfg, task_desc: str):
    ...
    while (save_request := save_req_queue.get()) is not None:
        ...
        save_demo(
            ...
            robot_cfg=robot_cfg,
            task_desc=task_desc,
            task_name=save_request.get("task_name"),
            generate_task_mask=save_request.get("generate_task_mask", False),
            task_mask_radius=save_request.get("task_mask_radius", 18),
            allow_center_fallback=save_request.get("task_mask_allow_center_fallback", False),
            task_mask_debug=save_request.get("task_mask_debug", False),
            camera_cfg=save_request.get("camera_cfg"),
        )
...
class DemoCollector:
    def __init__(
        ...
        self.generate_task_mask = generate_task_mask
        self.task_mask_radius = task_mask_radius
        self.task_mask_allow_center_fallback = task_mask_allow_center_fallback
        self.task_mask_debug = task_mask_debug
    ...
    def save(self, demo_idx: int, status: str):
        ...
        save_demo(
            ...
            generate_task_mask=self.generate_task_mask,
            task_mask_radius=self.task_mask_radius,
            allow_center_fallback=self.task_mask_allow_center_fallback,
            task_mask_debug=self.task_mask_debug,
...
def main():
    ...
    if args.generate_task_mask:
        camera_data_types.extend(["instance_seg", "instance_id_seg"])
    ...
    dr_level = args.level
    if dr_level > 0 and args.sim != "isaacsim":
        log.warning(
            f"DR level={dr_level} requested with sim='{args.sim}', but current DR pipeline depends on IsaacSim/USD. "
            "Falling back to level=0 for this run."
        )
        dr_level = 0
    ...
    collector = DemoCollector(
        ...
        task_name=args.task,
        generate_task_mask=args.generate_task_mask,
        task_mask_radius=args.task_mask_radius,
        task_mask_allow_center_fallback=args.task_mask_allow_center_fallback,
        task_mask_debug=args.task_mask_debug,
        camera_cfg={"pos": camera.pos, "look_at": camera.look_at, "intrinsics": camera.intrinsics},
    )
    pbar = tqdm(total=args.num_demo_success, desc="Collecting successful demos")
    ...
    if len(demo_idxs) == 0:
        log.warning(
            f"No available demo index in [{args.demo_start_idx}, {max_demo}). "
            "All demos are already marked as success/failed for current run mode."
        )
        collector.final()
        env.close()
        return

    if len(demo_idxs) < env.handler.num_envs:
        log.warning(
            f"Only {len(demo_idxs)} demo(s) available but num_envs={env.handler.num_envs}. "
            "Please reduce --num_envs or clear existing demo status to continue."
        )
        collector.final()
        env.close()
        return
    ...
    def _apply_dr_for_demo(demo_idx: int, env_id: int, *, is_initial: bool):
        if randomization_manager is None or dr_level <= 0:
            return
        randomization_manager.apply_randomization(demo_idx, is_initial=is_initial)
    ...
    for env_id, demo_idx in enumerate(demo_idxs):
        _apply_dr_for_demo(demo_idx, env_id, is_initial=True)
    ...
    while not all(finished):
        ...
        for env_id in success.nonzero().squeeze(-1).tolist():
            ...
            if not run_out[env_id] and steps_after_success[env_id] < args.tot_steps_after_success:
                ...
                if (not stop_flag) and (demo_indexer.next_idx < max_demo):
                    ...
                    _apply_dr_for_demo(new_demo_idx, env_id, is_initial=False)
        ...
        for env_id in (time_out | torch.tensor(run_out, device=time_out.device)).nonzero().squeeze(-1).tolist():
            ...
            if failure_count[env_id] < try_num:
                ...
                _apply_dr_for_demo(demo_idx, env_id, is_initial=False)
